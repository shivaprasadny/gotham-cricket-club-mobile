...
const handleUpdateFee = async () => {
  ...
  try {
    ...
    let finalSplits: { userId: number; amount: number }[] = [];
    ...
    if (billingMode === "FIXED_PER_PERSON") {
      ...
      finalSplits = selectedSplits.map((item) => ({
        userId: item.userId,
        amount: Number(amount),
      }));
    } else {
      finalSplits = selectedSplits
        .map((item) => ({
          userId: item.userId,
    ...
    const totalAmount = finalSplits.reduce(
      (sum, item) => sum + Number(item.amount),
      0
    );

   const response = await updateSplitFee(feeId, {
  title: title.trim(),
  feeType,
  amount: totalAmount,
...
const loadFeeData = async () => {
  try {
    ...
    if (fee.assignments && Array.isArray(fee.assignments)) {
     const existingSplits = fee.assignments.map((assignment: any) => ({
  userId: assignment.userId,
  fullName: assignment.fullName,
  amount: String(assignment.amount || ""),
}));

      setSelectedSplits(existingSplits);

      const amounts = existingSplits.map((x: any) => Number(x.amount));

      const allSame =
        amounts.length > 0 &&
        amounts.every((x: number) => x === amounts[0]);

      if (allSame) {
        setBillingMode("FIXED_PER_PERSON");
        setAmount(String(amounts[0]));
      } else {
        setBillingMode("CUSTOM_SPLIT");
        setAmount("");
      }
    }
  } catch (error: any) {
